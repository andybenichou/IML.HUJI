...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        i = 0
        while i < self.iterations_:

            # Invote base learner ht
            self.models_.append(self.wl_().fit(X, y * self.D_))

            y_pred = self.models_[-1]._predict(X)

            # Compute ε
            eps = np.sum(0.5 * np.abs(y_pred - y) * self.D_)

            # Set wt
            self.weights_.append(np.log((1 / eps) - 1) / 2)

            # Update sample weights D
            self.D_ = self.D_ * (np.exp(-y * self.weights_[-1] * y_pred))

            # Normalize weights D(t+1)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        models_pred = np.array([x.predict(X) for x in self.models_[:T]])

        sums = np.sum(self.weights_[t] * models_pred[t] for t in range(T))
